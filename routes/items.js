...
router.get("/items", authChecks.authRole(ROLE.KUPAC), (req, res) => {
...
router.get("/items/:id", authChecks.authRole(ROLE.KUPAC), (req, res) => {
...
router.delete("/items/:id", authChecks.authRole(ROLE.ADMIN), (req, res) => {
  queries.deleteItemById(req.params.id, (error, results, fields) => {
    if (error) {
      res.writeHead(500);
      res.write(JSON.stringify({ error: "Item not found or has dependency" }));
    } else {
      res.writeHead(200);
...
router.put("/items/:id", authChecks.authRole(ROLE.ADMIN), async (req, res) => {
  let item = {};
...
router.post("/items", authChecks.authRole(ROLE.ADMIN), async (req, res) => {
  try {
    let item = {};
    if (req.body.naziv !== undefined) {
      item.naziv = htmlEncode(req.body.naziv);
